...
app.use((req, res, next) => {
    ...
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, PATCH, DELETE, OPTIONS'); // allow GET, POST, etc. methods
...
app.get('/all', async (req, res, next) => {
    const imageFiles = await getDb().collection('images').find().sort(['_id', -1]).toArray();
...
app.delete('/delete-image', async (req, res, next) => {
    ...
    try {
        const deletion = await getDb()
            .collection('images')
            .deleteOne({ path: __dirname + '/images/' + decodedImage.split('/')[12] });
        if (deletion) {
            const imageFiles = await getDb().collection('images').find().sort(['_id', -1]).toArray();
